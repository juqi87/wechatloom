...
> 状态：v0.1 已完成；v0.2 已在 `0.2.0-dev` 完成；v0.3 已在 `0.3.0-dev` 开始但微信草稿尚未实现；v0.9 Skill 管理切片已通过自动化验证；V1.0 稳定化轨已启动 JSON envelope 契约冻结，但各版本退出条件仍不得跳过。
...
> 当前进度：已实现内嵌 Skill 的 `skill status/install/update codex`、来源版本与文件哈希清单、显式写入门禁、同目录 staging 和更新回滚；跨平台发布包与安装器尚未开始。
...
> 当前进度：已发布 Draft 2020-12 JSON envelope schema，并以真实 CLI 成功、运行失败和参数错误响应执行契约测试；未满足的发布门槛继续保留。
