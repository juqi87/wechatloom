---
name: wechatloom
description: Orchestrate the local WeChatLoom CLI to turn Markdown into themed, deterministic WeChat Official Account HTML with component layout, local images, formulas, Mermaid diagrams, interactive mobile preview, and PNG snapshots. Use when the user asks to convert or format a .md file for 微信公众号/WeChat Official Accounts, choose a WeChat theme, arrange an article with reusable components, inspect publishing readiness, preview mobile layouts, or prepare for a future WeChat draft; report that v0.2 has no remote publishing command.
---
...
- The current `0.2.x` implementation stops at local build artifacts and has no WeChat remote-write command.
...
- Do not add network access to a local-only build. Built-in themes, formulas, Mermaid, images, preview, and snapshots are local.
